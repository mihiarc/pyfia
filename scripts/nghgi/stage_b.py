...
1. pyfia's ``stock_change`` covers **only condition-level pools** —
   ...
   of national flux) are deferred to a tree-level GRM decomposition.
   The per-state residual ``EPA − pyfia_condition`` is the expected
...
Usage:
    uv run python scripts/nghgi/stage_b.py
    uv run python scripts/nghgi/stage_b.py --states GA,SC,FL
    uv run python scripts/nghgi/stage_b.py --db-dir /path/to/fiadb
...
from _compile import load_state_flux_targets  # noqa: E402
from _paths import resolve_db_dir  # noqa: E402

from pyfia import FIA  # noqa: E402
from pyfia.carbon.stock_change import stock_change  # noqa: E402
...
def run_state(state: str, *, db_dir: Path) -> dict | None:
    ...
    total_st = (
        ...
        if "CARBON_CHANGE_TOTAL" in sc.columns
        else 0.0
    )
    n_plots = int(sc["N_PLOTS"][0]) if "N_PLOTS" in sc.columns and len(sc) > 0 else 0

    # stock_change already annualizes by REMPER. Convert to MMT C and
    # negate to match EPA's sign convention (negative = net uptake).
...
def main() -> int:
    ...
    print("    pyfia: condition-level stock_change (4 pools)")
    print("    EPA:   Table A-208 all-pools state flux, 2022")
    print("    Expected residual: ~70-80% (= tree-level flux, not yet reproduced)")
    print(f"    State DB directory: {db_dir}\n")

    rows: list[dict] = []
    failures: list[tuple[str, str]] = []
    for i, st in enumerate(states, 1):
        r = run_state(st, db_dir=db_dir)
        if r is None:
    ...
    epa_df = load_state_flux_targets().rename(
        {
            "state": "STATE",
    ...
    cmp = cmp.with_columns(
        (pl.col("EPA_FLUX_MMT_C") - pl.col("pyfia_condition_flux_mmt_c")).alias(
    ...
    print(f"  Tree pools account for:      {pct_tree:.1f}% of EPA total")

    print("\n  Note: EPA Table A-208 total reported in Annex 3.13: -189.3 MMT C/yr")
    print(f"  Our CONUS-48 EPA sum:        {epa_total:+.1f} MMT C/yr")
    print("  (delta from -189.3 reflects HI/AK/territories in EPA total)\n")
    ...
    print(
        "(expected to be low — pyfia condition flux is only part of EPA's "
