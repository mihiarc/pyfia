...
def growth(db: Union[FIA, str],
    ...
    recruitment = _calculate_recruitment(
        ...
        component_col, micr_grow_col, subp_grow_col,
        grpBy, bySpecies
    )
    ...
    dia_growth = _calculate_diameter_growth(
        ...
        ppsa, pop_stratum, component_col,
        grpBy, bySpecies
    )
...
def _calculate_recruitment(tree_grm_component, tree_grm_begin, ppsa, pop_stratum,
                         component_col, micr_grow_col, subp_grow_col,
                         grpBy, bySpecies):
    ...
    recruitment = _aggregate_and_expand(ingrowth, 'RECR_TPA_ADJ', 'RECR_TPA', 
                                      grpBy, bySpecies)
    
    return recruitment


def _calculate_diameter_growth(tree_grm_component, tree_grm_begin, tree_grm_midpt,
                              ppsa, pop_stratum, component_col,
                              grpBy, bySpecies):
    ...
    dia_growth = _aggregate_mean(survivors, 'DIA_GROWTH_ADJ', 'DIA_GROWTH',
                                grpBy, bySpecies)
    
    return dia_growth


def _calculate_volume_growth(tree_grm_component, tree_grm_begin, tree_grm_midpt,
                           ppsa, pop_stratum, component_col,
                           grpBy, bySpecies):
    ...
    survivors = survivors.join(
        tree_grm_midpt.select(['TRE_CN', 'VOLCFNET']).rename({'VOLCFNET': 'VOLCFNET_MID'}),
        on='TRE_CN',
        how='left'
    )
...
def _calculate_biomass_growth(tree_grm_component, tree_grm_begin, tree_grm_midpt,
                             ppsa, pop_stratum, component_col,
                             grpBy, bySpecies):
...
def _aggregate_and_expand(data, value_col, output_col, grpBy, bySpecies):
    ...
    plot_data = data.group_by(plot_agg).agg(
        pl.col(value_col).sum().alias(f'PLOT_{output_col}')
    )
    
    # Get all GRM plots
    all_plots = data.select('PLT_CN').unique()
