...
def example_biomass_formatting():
    ...
    formatted = formatter.format_output(
        raw_biomass, variance=False, totals=True, group_cols=["SPCD"], year=2023
    )
...
def example_custom_formatter():
            ...
            if "AREA_PERC" in df.columns:
                df = df.with_columns(
                    [
                        ...
                        (pl.col("AREA_PERC") - 1.96 * pl.col("AREA_PERC_SE")).alias(
                            "CI_LOWER"
                        ),
                        (pl.col("AREA_PERC") + 1.96 * pl.col("AREA_PERC_SE")).alias(
                            "CI_UPPER"
    ...
    formatted = custom_formatter.format_output(raw_data, variance=False, year=2023)
