...
class TestGRMValidation:
    ...
    def test_growth_volume(self, fia_db, evalidator_client):
        ...
        with FIA(fia_db) as db:
            ...
            pyfia_growth, pyfia_se, pyfia_plot_count = extract_grm_estimate(
                result, "growth"
            )
        ...
        assert values_match(
            pyfia_growth, ev_result.estimate, rel_tol=DATA_SYNC_TOLERANCE
        ), (
        ...
        assert se_values_match(
            pyfia_se, ev_result.sampling_error, rel_tol=SE_TOLERANCE_GRM
        ), (
        ...
        if pyfia_plot_count is not None:
            ...
            if plot_diff > 0:
                print(
                    f"  Note: {plot_diff} zero-contribution plots counted differently"
                )
    ...
    def test_mortality_volume(self, fia_db, evalidator_client):
        ...
        with FIA(fia_db) as db:
            ...
            pyfia_mort, pyfia_se, pyfia_plot_count = extract_grm_estimate(
                result, "mortality"
            )
    ...
    def test_removals_volume(self, fia_db, evalidator_client):
        ...
        with FIA(fia_db) as db:
            ...
            pyfia_rem, pyfia_se, pyfia_plot_count = extract_grm_estimate(
                result, "removals"
            )
