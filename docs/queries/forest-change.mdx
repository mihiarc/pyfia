---
title: Forest change
description: "Forest area change over time using the subplot condition-change matrix and remeasurement linkages."
---

Change analysis links current and previous conditions through `PLOT.PREV_PLT_CN` and the `SUBP_COND_CHNG_MTRX` table, using a change evaluation (EVALID ending in `03`).

## Missouri — forest area change by type group

<Info>**EVALID:** 292403 (Missouri 2024 change) · **Focus:** net forest area by type group (oak/hickory dominates at ~11.8M acres) · [source](https://github.com/mihiarc/pyfia/blob/main/reference/queries/forest_change/missouri_forest_type_change.sql)</Info>

The change matrix tracks the proportion of each condition that changed between measurements (`SUBPTYP_PROP_CHNG`, divided by 4 for a per-plot basis).
...
FROM (
    ...
    LEFT JOIN (
        SELECT ref_forest_type.VALUE AS FORTYPC,
    ...
    ) REF_FORTYGPCDSQ ON REF_FORTYGPCDSQ.FORTYPC = cond.fortypcd
    WHERE
    ...
    GROUP BY
        ...
        REF_FORTYGPCDSQ.FORTYGPCD, REF_FORTYGPCDSQ.MEANING, pop_stratum.evalid
) subquery
GROUP BY GRP1, GRP2
ORDER BY GRP1, GRP2;
```
...
<Tip>The Python equivalent is [`area_change()`](/api/pyfia-estimation-estimators-area_change).</Tip>
