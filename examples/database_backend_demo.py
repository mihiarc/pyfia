...
def demo_backend_autodetection(db_path: Path):
    ...
    trees = reader.read_table(
        "TREE", columns=["CN", "SPCD", "DIA", "STATUSCD"], where="STATUSCD = 1", limit=5
    )
...
def demo_fia_class_with_backend(db_path: Path, engine: str = None):
    ...
    if evalids:
        ...
        state_counts = (
            plots.group_by("STATECD")
            .agg(pl.count().alias("plot_count"))
...
def demo_performance_options(db_path: Path):
    ...
    if db_path.suffix.lower() in [".duckdb", ".ddb"]:
        ...
        reader = FIADataReader(db_path, engine="duckdb", memory_limit="8GB", threads=4)
    ...
    # SQLite with timeout configuration
        ...
        reader = FIADataReader(db_path, engine="sqlite", timeout=60.0)
...
def main():
    ...
    if not available_dbs:
        console.print(
            "[red]No FIA databases found. Please adjust the paths in main()[/red]"
        )
        return
