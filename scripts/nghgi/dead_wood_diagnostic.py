...
Usage:
    uv run python scripts/nghgi/dead_wood_diagnostic.py
    uv run python scripts/nghgi/dead_wood_diagnostic.py --states GA,OR,TX,WA
    uv run python scripts/nghgi/dead_wood_diagnostic.py --db-dir /path/to/fiadb
...
from pyfia.carbon.downed_dead import downed_dead  # noqa: E402
from pyfia.carbon.standing_dead import standing_dead  # noqa: E402
from pyfia.estimation.estimators.carbon_pools import carbon_pool  # noqa: E402
...
def diagnose_state(state: str, *, db_dir: Path) -> dict:
    ...
    return {
        ...
        "A_over_B_pct": (
            100.0 * (a_total_st - b_total_st) / b_total_st
            if b_total_st
            else float("nan")
        ),
    }
...
def main() -> int:
    ...
    rows = []
    for st in states:
        r = diagnose_state(st, db_dir=db_dir)
        rows.append(r)
        print(
    ...
    print(
        f"  Standing dead delta (A − B): {a_sum - b_sum:+.1f} MMT C "
