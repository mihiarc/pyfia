...
This estimator reads ``COND.CARBON_LITTER`` (short tons per acre), then
runs it through pyFIA's post-stratified aggregation pipeline to produce
per-acre and population estimates that match EVALIDator.

Litter has no above-ground / below-ground split; the single
``CARBON_LITTER`` column represents the total pool.

National magnitude: ~5,200 Tg C total litter, roughly 6 % of total
forest ecosystem carbon (GTR-NRS-154, Table 2).

Public API: :func:`litter`.  See its docstring for parameters,
examples, and the pool semantics.

References
----------
- Domke, G.M.; Perry, C.H.; Walters, B.F.; et al. (2016). Estimating
...
class LitterEstimator(BaseEstimator):
    """Litter carbon estimator.

    Reads pre-computed ``COND.CARBON_LITTER`` from the FIADB and
    ...
    def get_cond_columns(self) -> list[str]:
        ...
        if "CARBON_LITTER" not in cols:
            cols.append("CARBON_LITTER")
        return cols
    ...
    def calculate_values(self, data: pl.LazyFrame) -> pl.LazyFrame:
        """Alias CARBON_LITTER to CARBON_ACRE."""
        cond_carbon = pl.col("CARBON_LITTER").cast(pl.Float64).fill_null(0.0)
...
def litter(
    ...
    """
    Estimate litter carbon from FIA data.

    Litter encompasses all organic material on the forest floor including
    decomposing leaves, needles, fine woody debris (< 7.62 cm / 3 in
    diameter), and humus (duff).  Carbon density is estimated from the
    Domke et al. (2016) model, parameterised on geographic area, forest
    type group, and stand age.  The FIADB pre-computes condition-level
    carbon density and stores it in ``COND.CARBON_LITTER``.

    Nationally, litter carbon totals ~5,200 Tg C, representing ~6 % of
    total forest ecosystem carbon (GTR-NRS-154, Table 2).  Per-acre
    values are typically 3-10 short tons/acre depending on forest type
    and region.
    ...
    pool : {'total'}, default 'total'
        Carbon pool to estimate.  Litter has no above-ground /
    ...
    -------
    pl.DataFrame
        Litter carbon estimates with the following columns:
    ...
    **Methodology**
    ...
    Litter is not directly measured on standard FIA plots in a manner
    that produces per-condition densities.  Instead, the FIADB
    pre-computes carbon density per condition using the Domke et al.
    (2016) model, which uses empirical relationships between litter
    carbon stocks and forest type, stand age, and geographic region.
    ...
    **No AG/BG split**
    ...
    Litter has no above-ground / below-ground partitioning.  The
    ``CARBON_LITTER`` column represents the entire pool.

    Examples
    --------
    Total litter carbon per acre on forestland:

    >>> results = litter(db, pool="total")
    >>> print(f"Carbon: {results['CARBON_ACRE'][0]:.3f} tons/acre")

    Litter carbon by forest type group:

    >>> results = litter(db, pool="total", grp_by="FORTYPGRPCD")

    Litter carbon on timberland with standard errors:
    ...
    References
    ----------
    .. [1] Domke, G.M.; Perry, C.H.; Walters, B.F.; et al. (2016).
       Estimating litter carbon stocks on forest land in the United
    ...
    if pool != "total":
        raise ValueError(
            f"Invalid pool '{pool}' for litter. "
            f"Only 'total' is supported — litter has no AG/BG split."
        )
    ...
    if most_recent and db.evalid is None:
        ...
        ensure_evalid_set(db, eval_type="VOL", estimator_name="litter")
    ...
    try:
        estimator = LitterEstimator(db, config)
