---
title: Volume
description: "Net merchantable bole volume by diameter class, EVALIDator-validated."
---

Volume queries expand `TPA_UNADJ × VOLCFNET` with diameter-based adjustment factors, restricted to merchantable timber species (≥5" DBH, non-woodland) on timberland.

## California — net merchantable volume by diameter class

<Info>**EVALID:** 62101 (California 2021) · **Result:** 67.05 billion cubic feet (4,188.7 cu ft/acre) · [source](https://github.com/mihiarc/pyfia/blob/main/reference/queries/volume/california_volume_by_diameter.sql)</Info>
...
WHERE
    ...
    AND TREE.DIA >= 5.0                     -- Merchantable timber
    AND REF_SPECIES.WOODLAND = 'N'          -- Timber species
    AND pop_stratum.rscd = 26               -- California
    AND pop_stratum.evalid = 62101
GROUP BY diameter_class
ORDER BY MIN(tree.dia);
```

</Accordion>

<Tip>The Python equivalent is [`volume()`](/api/pyfia-estimation-estimators-volume) with `by_size_class=True`.</Tip>
