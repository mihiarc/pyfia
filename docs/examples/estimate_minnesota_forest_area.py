...
def estimate_minnesota_forest_area(db_path: str = "fia.duckdb"):
    ...
    with Progress(
        ...
        task = progress.add_task(
            "[bold blue]Loading Minnesota FIA data...[/bold blue]", total=None
        )
        db = FIA(db_path)
        ...
        # 1. Total forest area
        task = progress.add_task(
            "[green]Calculating total forest area...[/green]", total=None
        )
        ...
        task = progress.add_task(
            "[green]Analyzing land type distribution...[/green]", total=None
        )
        ...
        # 3. Timberland area
        task = progress.add_task(
            "[green]Calculating timberland area...[/green]", total=None
        )
        ...
        task = progress.add_task(
            "[green]Analyzing forest ownership...[/green]", total=None
        )
        ...
        task = progress.add_task(
            "[green]Analyzing forest type distribution...[/green]", total=None
        )
        ...
        task = progress.add_task(
            "[green]Analyzing forest age distribution...[/green]", total=None
        )
        ...
        task = progress.add_task(
            "[green]Calculating reserved forest area...[/green]", total=None
        )
    ...
    if not total_forest.is_empty():
        ...
        console.print(
            f"   Forest area: {row['AREA']:,.0f} acres ({row['AREA_PERC']:.1f}% of total land)"
        )
    ...
    if not timberland.is_empty():
        ...
        console.print(
            f"   Timberland area: {row['AREA']:,.0f} acres ({row['AREA_PERC']:.1f}% of total)"
        )
        console.print(f"   Standard error: ±{row['AREA_PERC_SE']:.2f}%")
    ...
    by_forest_type = by_forest_type.with_columns(
        ...
        .alias("FOREST_TYPE_GROUP")
    )

    # Group by forest type group and sum areas
    grouped_types = (
        by_forest_type.group_by("FOREST_TYPE_GROUP")
        .agg([pl.sum("AREA").alias("AREA"), pl.sum("AREA_PERC").alias("AREA_PERC")])
    ...
    for row in grouped_types.iter_rows(named=True):
        table3.add_row(
            row["FOREST_TYPE_GROUP"], f"{row['AREA']:,.0f}", f"{row['AREA_PERC']:.1f}%"
        )
    ...
    by_stand_age_filtered = by_stand_age_filtered.with_columns(
        pl.when(pl.col("STDAGE") <= 20)
        .then(pl.lit("0-20"))
        .when(pl.col("STDAGE") <= 40)
        .then(pl.lit("21-40"))
        .when(pl.col("STDAGE") <= 60)
        .then(pl.lit("41-60"))
        .when(pl.col("STDAGE") <= 80)
        .then(pl.lit("61-80"))
        .when(pl.col("STDAGE") <= 100)
        .then(pl.lit("81-100"))
    ...
    # Group by age class
    age_class_summary = by_stand_age_filtered.group_by("AGE_CLASS").agg(
        [pl.sum("AREA").alias("AREA"), pl.sum("AREA_PERC").alias("AREA_PERC")]
    )
    ...
    for age_class in age_order:
        ...
        if not rows.is_empty():
            ...
            table4.add_row(age_class, f"{row['AREA']:,.0f}", f"{row['AREA_PERC']:.1f}%")
    ...
    if not total_forest.is_empty() and not timberland.is_empty():
        ...
        reserved_acres = (
            reserved_forest.row(0, named=True)["AREA"]
            if not reserved_forest.is_empty()
            else 0
        )
        ...
        console.print(
            f"   Reserved forest: {reserved_acres:,.0f} acres ({reserved_acres / total_forest_acres * 100:.1f}%)"
        )
        console.print(
            f"   Other forest: {total_forest_acres - timberland_acres - reserved_acres:,.0f} acres"
        )
