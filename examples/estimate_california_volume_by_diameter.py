...
def estimate_california_merchantable_volume(db_path: str = "fia.duckdb"):
    ...
    with Progress(
        ...
        task = progress.add_task(
            "[bold blue]Loading California FIA data...[/bold blue]", total=None
        )
        db = FIA(db_path)
        ...
        task = progress.add_task(
            "[green]Calculating total merchantable volume...[/green]", total=None
        )
        total_volume = volume(
            db,
        ...
        task = progress.add_task(
            "[green]Analyzing volume by diameter class...[/green]", total=None
        )
        ...
        task = progress.add_task(
            "[green]Calculating sawlog volume by diameter class...[/green]", total=None
        )
        ...
        task = progress.add_task(
            "[green]Analyzing volume by species...[/green]", total=None
        )
        ...
        task = progress.add_task(
            "[green]Analyzing volume by ownership...[/green]", total=None
        )
    ...
    console.print(
        "[dim]All volumes are net cubic feet of growing stock on timberland[/dim]\n"
    )
    ...
    if not total_volume.is_empty():
        ...
        console.print(
            f"   Standard error: ±{row['VOLCFNET_ACRE_SE']:.2f} cubic feet/acre"
        )
        if "VOL_TOTAL_BOLE_CF_ACRE" in row:
            console.print(
                f"   Total volume: {row['VOL_TOTAL_BOLE_CF_ACRE']:,.0f} cubic feet"
            )
    ...
    total_vol = (
        volume_by_diameter["VOL_TOTAL_BOLE_CF_ACRE"].sum()
        if not volume_by_diameter.is_empty()
        and "VOL_TOTAL_BOLE_CF_ACRE" in volume_by_diameter.columns
        else 0
    )
    ...
    total_sawlog = (
        sawlog_volume["VOL_TOTAL_SAW_CF_ACRE"].sum()
        if not sawlog_volume.is_empty()
        and "VOL_TOTAL_SAW_CF_ACRE" in sawlog_volume.columns
        else 0
    )
    ...
    vol_col = (
        "VOL_TOTAL_BOLE_CF_ACRE"
        if "VOL_TOTAL_BOLE_CF_ACRE" in volume_by_species.columns
        else "VOLCFNET_ACRE"
    )
    ...
    species_total = (
        volume_by_species[vol_col].sum() if vol_col in volume_by_species.columns else 0
    )
    ...
    ownership_pivot = volume_by_ownership.with_columns(
        pl.col("OWNGRPCD")
        .map_elements(get_ownership_group_name, return_dtype=pl.Utf8)
        .alias("Owner")
    ).pivot(
        values="VOLCFNET_ACRE", index="SIZE_CLASS", on="Owner", aggregate_function="sum"
    )
    ...
    if not volume_by_diameter.is_empty():
        ...
        dist_total = (
            volume_by_diameter["VOL_TOTAL_BOLE_CF_ACRE"].sum()
            if "VOL_TOTAL_BOLE_CF_ACRE" in volume_by_diameter.columns
            else volume_by_diameter["VOLCFNET_ACRE"].sum()
        )
