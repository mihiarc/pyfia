...
This estimator reads ``COND.CARBON_DOWN_DEAD`` (short tons per acre),
...
Downed dead wood has no above-ground / below-ground split; the single
``CARBON_DOWN_DEAD`` column represents the total pool.

National magnitude: ~2,700 Tg C total downed dead wood, roughly 3 % of
total forest ecosystem carbon (GTR-NRS-154, Table 2).

Public API: :func:`downed_dead`.  See its docstring for parameters,
examples, and the pool semantics.

References
----------
- Domke, G.M.; Woodall, C.W.; Smith, J.E. (2013). Accounting for density
...
class DownedDeadEstimator(BaseEstimator):
    """Downed dead wood carbon estimator.

    Reads pre-computed ``COND.CARBON_DOWN_DEAD`` from the FIADB and
    ...
    def get_cond_columns(self) -> list[str]:
        ...
        if "CARBON_DOWN_DEAD" not in cols:
            cols.append("CARBON_DOWN_DEAD")
        return cols
    ...
    def calculate_values(self, data: pl.LazyFrame) -> pl.LazyFrame:
        """Alias CARBON_DOWN_DEAD to CARBON_ACRE."""
        cond_carbon = pl.col("CARBON_DOWN_DEAD").cast(pl.Float64).fill_null(0.0)
    ...
    def aggregate_results(self, data: pl.LazyFrame | None) -> AggregationResult:
        ...
        validate_required_columns(
            data, ["PLT_CN", "CARBON_ACRE"], "downed dead carbon data"
        )
...
def downed_dead(
    ...
    """
    Estimate downed dead wood carbon from FIA data.

    Downed dead wood (coarse woody debris) encompasses all dead wood
    lying on the forest floor with a diameter >= 7.62 cm (3 in) at the
    point of intersection with a sampling transect.  Carbon density is
    estimated from the Domke et al. (2013) model, parameterised on
    geographic area, forest type group, and stand age.  The FIADB
    pre-computes condition-level carbon density and stores it in
    ``COND.CARBON_DOWN_DEAD``.

    Nationally, downed dead wood carbon totals ~2,700 Tg C, representing
    ~3 % of total forest ecosystem carbon (GTR-NRS-154, Table 2).
    Per-acre values are typically 1-5 short tons/acre depending on forest
    type and region.
    ...
    pool : {'total'}, default 'total'
        Carbon pool to estimate.  Downed dead wood has no above-ground /
    ...
    -------
    pl.DataFrame
        Downed dead wood carbon estimates with the following columns:
    ...
    soil_organic : Estimate soil organic carbon.
    ...
    **Methodology**
    ...
    Downed dead wood is not directly measured on standard FIA plots in a
    manner that produces per-condition densities.  Instead, the FIADB
    pre-computes carbon density per condition using the Domke et al.
    (2013) model, which accounts for density reductions across decay
    classes and uses regional CWD volume equations.
    ...
    **No AG/BG split**
    ...
    Unlike understory vegetation, downed dead wood has no above-ground /
    below-ground partitioning.  The ``CARBON_DOWN_DEAD`` column represents
    the entire pool.
    ...
    >>> results = downed_dead(db, pool="total")
    >>> print(f"Carbon: {results['CARBON_ACRE'][0]:.3f} tons/acre")

    Downed dead wood carbon by forest type group:

    >>> results = downed_dead(db, pool="total", grp_by="FORTYPGRPCD")

    Downed dead wood carbon on timberland with standard errors:
    ...
    References
    ----------
    .. [1] Domke, G.M.; Woodall, C.W.; Smith, J.E. (2013). Accounting for
       density reductions in dead wood decay classes. Forest Ecology and
    ...
    if pool != "total":
        raise ValueError(
            f"Invalid pool '{pool}' for downed dead wood. "
            f"Only 'total' is supported — downed dead wood has no AG/BG split."
        )
    ...
    if most_recent and db.evalid is None:
        ...
        ensure_evalid_set(db, eval_type="VOL", estimator_name="downed_dead")
    ...
    try:
        estimator = DownedDeadEstimator(db, config)
