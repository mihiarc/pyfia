...
def growth_direct(db: Union[FIA, str],
    ...
    plots_with_valid_components = tree_grm_component.filter(
        ~pl.col(component_col).is_in(['NOT USED', None])
    ...
    plots_with_trees = tree_grm_component.filter(
    ...
    # Combine all filters
    valid_grm_plots = plots_with_valid_components \
        .join(sampled_plots, on='PLT_CN', how='inner') \
    ...
    recruitment = _calculate_recruitment_direct(
        tree_grm_component, tree_grm_begin, ppsa, pop_stratum,
        component_col, land_suffix, all_grm_plots
    )
...
def _calculate_recruitment_direct(tree_grm_component, tree_grm_begin, ppsa, pop_stratum,
                                component_col, land_suffix, all_grm_plots):
    """Calculate recruitment using direct expansion."""
    
    # Filter to INGROWTH trees
    ingrowth = tree_grm_component.filter(
        pl.col(component_col) == 'INGROWTH'
    )
    
    # Simple approach: count trees per plot
    ingrowth_counts = ingrowth.group_by('PLT_CN').agg(
        pl.len().alias('n_ingrowth')
    )
    ...
    ingrowth_plots = ingrowth_plots.with_columns(
        (pl.col('n_ingrowth') / plot_size_acres * pl.col('ADJ_FACTOR_SUBP')).alias('ingrowth_tpa')
    )
    
    # Join with all plots and calculate expansion
    full_data = all_grm_plots.join(
        ingrowth_plots.select(['PLT_CN', 'ingrowth_tpa']),
        on='PLT_CN',
        how='left'
    ).with_columns(
        pl.col('ingrowth_tpa').fill_null(0.0)
    )
...
def _calculate_removals_direct(tree_grm_component, tree_grm_begin, ppsa, pop_stratum,
                              component_col, land_suffix, all_grm_plots):
    """Calculate removals (harvest) using direct expansion."""
    
    # Filter to CUT trees
    cut_trees = tree_grm_component.filter(
        pl.col(component_col).is_in(['CUT1', 'CUT2'])
    )
    ...
    cut_trees = cut_trees.join(
        tree_grm_begin.select(['TRE_CN', 'DIA']),
        on='TRE_CN',
        how='inner'
    )
    ...
    full_data = all_grm_plots.join(
        plot_remv,
        on=['PLT_CN', 'STRATUM_CN', 'EXPNS'],
        how='left'
    ).with_columns(
        pl.col('PLOT_REMV_TPA').fill_null(0.0)
    )
...
def _calculate_diameter_growth_direct(tree_grm_component, tree_grm_begin, ppsa, pop_stratum,
                                     component_col):
    """Calculate average diameter growth of survivor trees."""
    
    # Filter to SURVIVOR trees
    survivors = tree_grm_component.filter(
        pl.col(component_col) == 'SURVIVOR'
    )
...
def _get_beginning_tpa(tree_grm_begin, ppsa, pop_stratum, all_grm_plots):
    ...
    # Count trees by plot
    tree_counts = tree_grm_begin.group_by('PLT_CN').agg(
        pl.len().alias('n_trees')
    )
    
    # Join with plot data
    plot_trees = all_grm_plots.join(
        tree_counts,
        on='PLT_CN',
        how='left'
    ).with_columns(
    ...
    plot_trees = plot_trees.with_columns(
        (pl.col('n_trees') / (1/6)).alias('tpa')
    )
