"""
Lazy mortality estimation for pyFIA with optimized memory usage.

This module implements LazyMortalityEstimator which extends LazyBaseEstimator
...
class LazyMortalityEstimator(EstimatorProgressMixin, LazyBaseEstimator):
    """
    Lazy mortality estimator with optimized memory usage and performance.
    ...
    def __init__(self, db: Union[str, FIA], config: MortalityConfig):
        """
        Initialize the lazy mortality estimator.
    ...
    def estimate(self) -> pl.DataFrame:
        """
        Run lazy mortality estimation workflow.
        ...
        with self.progress_context():
            with self._track_operation(OperationType.COMPUTE, "Full mortality estimation", total=5):
                ...
                plot_wrapper = self._calculate_plot_mortality_lazy(prepared_wrapper, group_cols)
                self._update_progress(completed=3, description="Plot mortality calculated")
                
                # Step 4: Calculate stratum-level mortality lazily
                stratum_wrapper = self._calculate_stratum_mortality_lazy(plot_wrapper, group_cols)
                ...
                pop_stratum = self._get_pop_stratum_lazy()
    ...
    def _get_filtered_data_lazy(self) -> tuple[LazyFrameWrapper, LazyFrameWrapper]:
        """
    ...
    def _prepare_estimation_data_lazy(
        self,
        ...
        ppsa_wrapper = self._get_ppsa_lazy()
        
        # Get stratum data lazily
        pop_stratum_wrapper = self._get_pop_stratum_lazy()
    ...
    def _calculate_plot_mortality_lazy(
        self,
    ...
    def _calculate_stratum_mortality_lazy(
        self,
    ...
    def _get_ppsa_lazy(self) -> LazyFrameWrapper:
        """Get plot-stratum assignments lazily."""
    ...
    def _get_pop_stratum_lazy(self) -> LazyFrameWrapper:
...
def mortality_lazy(
    ...
    """
    Lazy mortality estimation with same API as mortality().
    
    This function provides identical interface to mortality() but uses
    lazy evaluation for improved performance. It offers:
    ...
    >>> 
    >>> # Basic usage - uses lazy evaluation automatically
    >>> db = FIA("fia.duckdb")
    >>> results = mortality_lazy(db, by_species=True)
    >>> 
    >>> # With progress tracking disabled
    >>> results = mortality_lazy(db, by_species=True, show_progress=False)
    >>> 
    >>> # Complex grouping with multiple parameters
    >>> results = mortality_lazy(
    ...     db,
    ...     by_species=True,
    ...     by_ownership=True,
    ...     mortality_type="both",
    ...
    # Use lazy estimator
    with LazyMortalityEstimator(db, final_config) as estimator:
...
def mortality(db: Union[str, FIA], **kwargs) -> pl.DataFrame:
    """
    Original mortality function - delegates to lazy implementation.
    
    This function maintains backward compatibility by delegating to
    the lazy implementation. All parameters are passed through.
    
    See mortality_lazy() for full documentation.
    """
    # Extract show_progress if provided, default to True
    show_progress = kwargs.pop("show_progress", True)
    
    # Call lazy implementation
    return mortality_lazy(db, show_progress=show_progress, **kwargs)